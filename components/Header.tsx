...
import { SunIcon, MoonIcon, AnimatedMenuIcon } from './icons';
...
const Header: React.FC<HeaderProps> = ({ onAboutClick, onProjectsClick, onSkillsClick, onPlaygroundClick, onBlogClick, onContactClick }) => {
  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-cosmic-light/80 dark:bg-cosmic-dark/80 backdrop-blur-sm shadow-lg' : 'bg-transparent'}`}>
      <nav className="container mx-auto px-6 py-4 flex justify-between items-center">
        ...
        <div className="md:hidden flex items-center">
          <button onClick={toggleTheme} className="p-2 rounded-full mr-2 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors" aria-label="Toggle theme">
             {theme === 'light' ? <MoonIcon /> : <SunIcon />}
          </button>
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-cosmic-light dark:bg-cosmic-dark py-4">
