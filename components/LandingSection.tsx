...
import ThreeCanvasLight from './ThreeCanvasLight';
...
const LandingSection: React.FC<LandingSectionProps> = ({ onCTAClick }) => {
  ...
  return (
    <section className="relative h-screen w-full flex flex-col justify-center items-center text-center overflow-hidden">
      {theme === 'dark' ? <ThreeCanvas /> : <ThreeCanvasLight />}
      <div className="relative z-10 p-6 space-y-6">
        <h1 ref={titleRef} className="text-5xl md:text-7xl lg:text-8xl font-black uppercase tracking-wide text-text-light dark:text-text-dark">
          Pedro Henrique Garcia
        </h1>
        <h2 ref={subtitleRef} className="max-w-3xl mx-auto text-lg md:text-xl text-gray-700 dark:text-gray-400 font-light">
          Desenvolvedor de Software e Criador de Soluções Inovadoras. Apaixonado por sistemas de baixo nível, IA e por transformar ideias em realidade.
        </h2>
        <button 
          ref={ctaRef}
          onClick={onCTAClick}
